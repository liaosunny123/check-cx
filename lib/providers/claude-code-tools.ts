/**
 * Claude Code CLI 完整 tools 定义模板
 *
 * 从真实的 Claude Code CLI 请求中提取，用于模拟 opus/sonnet 请求。
 * 包含全部 25 个 tool 定义，与 Claude Code 2.1.81 版本一致。
 */

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const CLAUDE_CODE_TOOLS: any[] = [
  {
    name: "Agent",
    description:
      ...
      properties: {
        questions: {
          ...
          items: {
            type: "object",
            properties: {
              question: { description: "The complete question to ask the user. Should be clear, specific, and end with a question mark.", type: "string" },
              header: { description: "Very short label displayed as a chip/tag (max 12 chars).", type: "string" },
              options: {
                ...
                items: {
                  type: "object",
                  properties: {
                    label: { description: "The display text for this option.", type: "string" },
                    description: { description: "Explanation of what this option means.", type: "string" },
                    preview: { description: "Optional preview content rendered when this option is focused.", type: "string" },
                  },
                  required: ["label", "description"],
                  additionalProperties: false,
                },
              },
              multiSelect: { description: "Set to true to allow the user to select multiple options.", default: false, type: "boolean" },
            },
        ...
        annotations: { description: "Optional per-question annotations from the user.", type: "object", propertyNames: { type: "string" }, additionalProperties: { type: "object", properties: { preview: { type: "string" }, notes: { type: "string" } }, additionalProperties: false } },
        metadata: { description: "Optional metadata for tracking and analytics purposes.", type: "object", properties: { source: { description: "Optional identifier for the source of this question.", type: "string" } }, additionalProperties: false },
      },
      required: ["questions"],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        description: { description: "A detailed description of what needs to be done", type: "string" },
        activeForm: { description: 'Present continuous form shown in spinner when in_progress (e.g., "Running tests")', type: "string" },
        metadata: { description: "Arbitrary metadata to attach to the task", type: "object", propertyNames: { type: "string" }, additionalProperties: {} },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      type: "object",
      properties: {
        taskId: { description: "The ID of the task to retrieve", type: "string" },
      },
      required: ["taskId"],
      additionalProperties: false,
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        addBlocks: { description: "Task IDs that this task blocks", type: "array", items: { type: "string" } },
        addBlockedBy: { description: "Task IDs that block this task", type: "array", items: { type: "string" } },
        owner: { description: "New owner for the task", type: "string" },
        metadata: { description: "Metadata keys to merge into the task. Set a key to null to delete it.", type: "object", propertyNames: { type: "string" }, additionalProperties: {} },
      },
      required: ["taskId"],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    input_schema: {
      ...
      type: "object",
      properties: {
        cron: { description: 'Standard 5-field cron expression in local time: "M H DoM Mon DoW".', type: "string" },
        prompt: { description: "The prompt to enqueue at each fire time.", type: "string" },
        recurring: { description: "true (default) = fire on every cron match until deleted or auto-expired after 7 days. false = fire once at the next match, then auto-delete.", type: "boolean" },
      },
